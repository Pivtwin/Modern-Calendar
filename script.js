...
function initCalendar() {
  // to get prev month days + current month all days + rem next month days
...
dateInput.addEventListener("input", (e) => {
  ...
  if (dateInput.value.length > 7) {
    // don't allow more than 7 charecters
...
addEventFrom.addEventListener("input", (e) => {
  ...
  // doesn't let user enter more than 5 characters
  if (addEventFrom.value.length > 2) {
...
// let's create function to add listener on days after rendered
function addListner() {
...
// let's create function to add events
addEventSubmit.addEventListener("click", () => {
  ...
  // also add an event class to the new added day if not already done
...
// let's create a function to remove click events
  ...
  if (e.target.classList.contains("event")) {
    ...
    eventsArr.forEach((event) => {
      if (
        ...
        // if no event remaing on that date remove complete day
...
// let's store events in local storage and get them from there
function saveEvents() {
